...
import { createVIPCheckoutSession, createBillingPortalSession, type VIPTier } from '@/actions/stripe'
...
export default function VIPPageClient({ initialVipDetails }: { initialVipDetails: VIPDetailsState }) {
  const [showModal, setShowModal] = useState(false)
  const [openFAQ, setOpenFAQ] = useState<number | null>(null)
  const [isLoading, setIsLoading] = useState(false)
  // Statut VIP résolu côté serveur (SSR) : plus de spinner ni de fetch au montage.
  const [vipDetails] = useState<VIPDetailsState>(initialVipDetails)
