...
// rendrait la requête dynamique). Lectures PUBLIQUES uniquement.
...
// Parties LIVE : FRAÎCHES à chaque visite (PAS de cache) pour que la carte hero du
// mockup montre une vraie partie en cours avec le bon timer. Le cache montrait parfois
// une partie déjà finie -> le client basculait en démo, rendu moche. Requête légère.
async function getLiveGames(): Promise<{
  ...
  const [gameResult, activeGamesResult] = await Promise.all([
    supabase
      .from('games')
  ...
  return { featuredGame, featuredItem, liveGames, prizes }
}

// Reste de la landing (gagnants, item de secours, stats) : CACHE 30 s -> le hero part
// vite même si la base est lente. Les gagnants se rafraîchissent ensuite en temps réel
// (useLandingRealtime).
const getCachedLandingData = unstable_cache(
  async () => {
    ...
    const [winnersResult, fallbackItemResult, totalItemsValueResult] = await Promise.all([
      supabase
        .from('winners')
      ...
      supabase
        ...
        .limit(1)
        .single(),
      supabase.from('items').select('retail_value', { count: 'exact' }),
    ])
    ...
    const featuredItemFallback: FeaturedItem | null = typedFallbackItem
      ? {
    ...
    const totalItemsValue = (typedItemsData || []).reduce((sum, item) => sum + (item.retail_value || 0), 0)
    const totalItemsCount = totalItemsValueResult.count || 0

    return {
      winners,
      featuredItemFallback,
      stats: {
...
export default async function LandingPage() {
  ...
  // Auth + reste (caché) + parties live (fraîches) en parallèle.
  const [{ data: { user } }, cached, live] = await Promise.all([
    supabase.auth.getUser(),
    getCachedLandingData(),
    getLiveGames(),
  ])

  const { winners, stats } = cached
  const { liveGames, featuredGame, prizes } = live
  const featuredItem = live.featuredItem ?? cached.featuredItemFallback
