  ...
  return (
    ...
    >
      ...
      <div className={`relative ${wide ? 'aspect-[21/10] sm:aspect-[11/4]' : 'aspect-[4/3]'} bg-gradient-to-br from-bg-tertiary to-bg-secondary overflow-hidden ${isEnded ? 'grayscale-[30%]' : ''}`}>
        ...
        {/* Badge « Débutants » : enchère réservée aux joueurs sans victoire (Lot G — confiance) */}
        {game.beginners_only && !isEnded && (
          <div
            className="absolute bottom-3 right-3 flex items-center gap-1 px-2.5 py-1 rounded-lg backdrop-blur-sm bg-neon-blue/90 shadow-[0_0_15px_rgba(60,203,255,0.45)]"
            title="Réservée aux joueurs sans victoire — laisse leur chance aux nouveaux"
          >
            ...
            <span className="text-[11px] font-bold text-white uppercase tracking-wide">Débutants</span>
          </div>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        <h2 className={`font-bold text-lg mb-1 truncate transition-colors ${isEnded ? 'text-white/60' : 'text-white group-hover:text-neon-purple'}`}>
          {game.item.name}
        </h2>
